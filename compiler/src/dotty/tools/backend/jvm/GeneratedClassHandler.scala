...
private[jvm] object GeneratedClassHandler {
  ...
  sealed abstract class WritingClassHandler(val javaExecutor: Executor) extends GeneratedClassHandler {
    ...
    def process(unit: GeneratedCompilationUnit): Unit = {
      val unitInPostProcess = new CompilationUnitInPostProcess(unit.classes, unit.sourceFile)(using unit.ctx)
 ...
 */
final class CompilationUnitInPostProcess(private var classes: List[GeneratedClass], val sourceFile: AbstractFile)(using Context) {
